    ...
    <x-slot name="header">
        <div class="flex flex-col md:flex-row md:justify-between md:items-center space-y-2 md:space-y-0">
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
                {{ __('Modifier la boutique') }}
            </h2>
            <div class="flex space-x-2">
                <a href="{{ route('shops.show', $shop) }}" class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 focus:bg-gray-700 active:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 transition ease-in-out duration-150">
                    ...
                    <form action="{{ route('shops.update', $shop) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="space-y-6">
                            ...
                            <div>
                                ...
                                <span class="text-red-500">*</span>
                                <x-text-input id="address" class="block mt-1 w-full" type="text" name="address" :value="old('address', $shop->address)" required />
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <span class="text-red-500">*</span>
                                    <x-text-input id="phone" class="block mt-1 w-full" type="text" name="phone" :value="old('phone', $shop->phone)" required />
                                    <x-input-error :messages="$errors->get('phone')" class="mt-2" />
                                </div>
                                <div>
                                    <x-input-label for="email" :value="__('Email')" />
                                    <span class="text-red-500">*</span>
                                    <x-text-input id="email" class="block mt-1 w-full" type="email" name="email" :value="old('email', $shop->email)" required />
                            ...
                            <div>
                                ...
                                @if($shop->logo_path)
                                    <div class="mt-2 mb-4">
                                        <img src="{{ asset('storage/' . $shop->logo_path) }}" alt="{{ $shop->name }}" class="max-h-24 block mb-1">
                                        <span class="text-sm text-gray-500 dark:text-gray-400">Logo actuel</span>
                                    </div>
                                @endif
                            ...
                            <div class="flex items-center">
                                <input id="is_active" name="is_active" type="checkbox" value="1" {{ old('is_active', $shop->is_active) ? 'checked' : '' }}
