                            ...
                            <div class="bg-white dark:bg-gray-700 rounded-lg shadow-sm border border-gray-200 dark:border-gray-600 p-4">
                                ...
                                <div class="mt-4">
                                    <div class="flex justify-between text-xs text-gray-500 dark:text-gray-400 mb-1">
                                        <span>{{ __('Dernière commission') }}</span>
                                        <span>{{ __('Paiement') }}</span>
                                    </div>
                                    @if($vendor->commission_stats['last_commission'])
                                        <div class="flex justify-between text-sm">
                                            <span class="text-gray-900 dark:text-gray-100">{{ $vendor->commission_stats['last_commission']->created_at->format('d/m/Y') }}</span>
                                            @if($vendor->commission_stats['last_commission']->is_paid)
                                                <span class="text-green-600 dark:text-green-400">{{ __('Payée') }}</span>
                                            @else
                                                <a href="#" class="text-amber-600 dark:text-amber-400 hover:underline" onclick="event.preventDefault(); document.getElementById('pay-form-{{ $vendor->id }}').submit();">
                                                    {{ __('Payer toutes') }}
                                                </a>
                                                <form id="pay-form-{{ $vendor->id }}" action="{{ route('commissions.pay-vendor', $vendor) }}" method="POST" class="hidden">
                                                    @csrf
                                                </form>
                                            @endif
                                        </div>
                                    @else
                                        <div class="text-sm text-gray-500 dark:text-gray-400">{{ __('Aucune commission') }}</div>
                                    @endif
                                </div>
                            </div>
                        @empty
