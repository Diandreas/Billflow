    ...
    <x-slot name="header">
        <div class="bg-gradient-to-r from-purple-600 to-indigo-500 py-3 px-3 rounded-lg shadow-sm mb-4">
            <div class="flex justify-between items-center">
                ...
                <a href="{{ route('commissions.create') }}" class="inline-flex items-center px-3 py-1 text-xs bg-white text-purple-700 rounded-md hover:bg-purple-50">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-3">
                    ...
                    <div class="mb-3 bg-gray-50 p-2 rounded-lg">
                        <form action="{{ route('commissions.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-2">
                            <div>
                                <label for="shop" class="block text-xs font-medium text-gray-700 mb-1">{{ __('Boutique') }}</label>
                                <select name="shop" id="shop" class="w-full text-xs rounded-md border-gray-300 shadow-sm focus:border-purple-500 focus:ring focus:ring-purple-200 focus:ring-opacity-50">
                            ...
                            <div>
                                ...
                                <select name="month" id="month" class="w-full text-xs rounded-md border-gray-300 shadow-sm focus:border-purple-500 focus:ring focus:ring-purple-200 focus:ring-opacity-50">
                            ...
                            <div>
                                ...
                                <select name="status" id="status" class="w-full text-xs rounded-md border-gray-300 shadow-sm focus:border-purple-500 focus:ring focus:ring-purple-200 focus:ring-opacity-50">
                            ...
                            <div class="md:flex md:flex-col md:justify-end">
                                <button type="submit" class="mt-4 inline-flex justify-center items-center px-3 py-1.5 bg-purple-600 border border-transparent rounded text-xs font-medium text-white hover:bg-purple-700">
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200 text-sm">
                            <thead class="bg-gray-50">
                                <tr>
                                    <th scope="col" class="px-3 py-1.5 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-1/6">{{ __('Référence') }}</th>
                                    <th scope="col" class="px-3 py-1.5 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-1/4">{{ __('Boutique') }}</th>
                                    <th scope="col" class="px-3 py-1.5 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-1/6">{{ __('Période') }}</th>
                                    <th scope="col" class="px-3 py-1.5 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-1/6">{{ __('Montant') }}</th>
                                    <th scope="col" class="px-3 py-1.5 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-1/6">{{ __('Statut') }}</th>
                                    <th scope="col" class="px-3 py-1.5 text-right text-xs font-medium text-gray-500 uppercase tracking-wider w-1/6">{{ __('Actions') }}</th>
                                </tr>
                            </thead>
                            <tbody class="bg-white divide-y divide-gray-200">
                                    ...
                                    <tr class="hover:bg-gray-50">
                                        ...
                                        <td class="px-3 py-1.5 whitespace-nowrap text-xs text-right">
                                            <div class="flex justify-end space-x-1">
                                                <a href="{{ route('commissions.show', $commission) }}" class="text-purple-600 hover:text-purple-900" title="{{ __('Voir') }}">
                                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
                                                        <path d="M10 12a2 2 0 100-4 2 2 0 000 4z" />
                                                        <path fill-rule="evenodd" d="M.458 10C1.732 5.943 5.522 3 10 3s8.268 2.943 9.542 7c-1.274 4.057-5.064 7-9.542 7S1.732 14.057.458 10zM14 10a4 4 0 11-8 0 4 4 0 018 0z" clip-rule="evenodd" />
                                                    </svg>
                                                </a>
                                                <a href="{{ route('commissions.edit', $commission) }}" class="text-purple-600 hover:text-purple-900" title="{{ __('Modifier') }}">
                                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
                                                        <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z" />
                                                    </svg>
                                                </a>
                                                @if ($commission->status !== 'paid')
                                                    <button type="button" onclick="confirmDelete('{{ $commission->id }}')" class="text-red-600 hover:text-red-900" title="{{ __('Supprimer') }}">
                                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
                                ...
                                @empty
                                    <tr>
                                        <td colspan="6" class="px-3 py-2 text-center text-gray-500 text-xs">{{ __('Aucune commission trouvée') }}</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
